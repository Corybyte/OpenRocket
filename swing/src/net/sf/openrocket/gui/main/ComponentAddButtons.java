...
//递归查询当前节点的父组件
    public static RocketComponent findComponent(RocketComponent rocket, RocketComponent targetComponent) {
        // 获取当前火箭的子组件
        List<RocketComponent> children = rocket.getChildren();

        // 遍历当前层级的子组件
        for (int i = 0; i < children.size(); i++) {
            RocketComponent child = children.get(i);

            // 如果找到目标组件
            if (child.equals(targetComponent)) {
                return child.getParent(); // 返回他的父级
            }

            // 如果当前组件有子组件，递归查找
            if (child.getChildCount() != 0) {
                RocketComponent result = findComponent(child, targetComponent);
                if (result != null) {
                    return result; // 如果找到目标，返回结果
                }
            }
        }
        return null;
...
//查询是第几个
    public void addComponent(RocketComponent rocket, RocketComponent targetComponent) {

        //父级组件查询
        List<RocketComponent> children = rocket.getChildren();
        int i=0;
        for (; i < children.size(); i++) {
            if (children.get(i)==targetComponent){
                break;
            }
        }
        build(rocket,i);
...
//            build(rocket,i);
//        }else {
//            System.out.println("需要在组件："+children.get(i-1)+"后插入");
//        }
    }
