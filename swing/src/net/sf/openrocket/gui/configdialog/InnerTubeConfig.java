...
public class InnerTubeConfig extends RocketComponentConfig {
    ...
    public InnerTubeConfig(OpenRocketDocument d, RocketComponent c, JDialog parent) {
        ...
        {//// CG calculation demonstration
            panel.add(new JLabel(trans.get("InnerTube.lbl.CgCalc") + ":"), "alignx left");
            JButton button = new JButton(trans.get("InnerTube.lbl.CgEnter"));
            panel.add(button, "spanx, wrap");
            button.addActionListener(e -> {
                JDialog dialog = new JDialog(this.parent, trans.get("InnerTube.lbl.CgCalc"));
        ...
        {//// MOI calculation demonstration
            panel.add(new JLabel(trans.get("InnerTube.lbl.MOICal") + ":"), "alignx left");
            JButton button = new JButton(trans.get("InnerTube.lbl.MOIEnter"));
            panel.add(button, "spanx, wrap");
            button.addActionListener(e -> {
                JDialog dialog = new JDialog(this.parent, trans.get("InnerTube.lbl.MOICal"));
                dialog.setSize(this.parent.getSize());
                dialog.setLocationRelativeTo(null);
                dialog.setLayout(new MigLayout("fill, gap 4!, ins panel, hidemode 3", "[]:5[]", "[]:5[]"));
                ...
                checkButton.addActionListener(e1 -> OpenRocket.eduCoderService.calculateMOI(request).enqueue(new Callback<>() {
                    ...
                    public void onResponse(@NotNull Call<Result> call, @NotNull Response<Result> response) {
                        ...
                        SwingUtilities.invokeLater(() -> {
                            checkResult.setText(trans.get("InnerTube.lbl.checkResult") + ": " + result.getResult());
                            answerLabel.setText(trans.get("InnerTube.lbl.answer") + ": " + component.getRotationalUnitInertia());
                        });
                    }
