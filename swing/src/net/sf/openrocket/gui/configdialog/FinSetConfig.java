...
public abstract class FinSetConfig extends RocketComponentConfig {
    ...
    private JPanel finTabPanel() {
        ...
        {//// MOI calculation demonstration
            panel.add(new JLabel(trans.get("FinSet.lbl.MOICal") + ":"));
            JButton button = new JButton(trans.get("FinSet.lbl.MOIEnter"));
            panel.add(button, "spanx, wrap");
            button.addActionListener(e -> {
                JDialog dialog = new JDialog(this.parent, trans.get("FinSet.lbl.MOICal"));
                dialog.setSize(this.parent.getSize());
                dialog.setLocationRelativeTo(null);
                dialog.setLayout(new MigLayout("fill, gap 4!, ins panel, hidemode 3", "[]:5[]", "[]:5[]"));
