...
public class RK4SimulationStepper extends AbstractSimulationStepper {
    ...
    private AccelerationData calculateAcceleration(RK4SimulationStatus status, DataStore store) throws SimulationException {
        ...
        if (store.flightConditions.getAOA() != 0 && store.flightConditions.getTheta() != 0) {
            ...
            request.setCDaxial(store.forces.getCDaxial());
            request.setRefArea(store.flightConditions.getRefArea());
            request.setRefLength(store.flightConditions.getRefLength());
            request.setCN(store.forces.getCN());
//			request.setRocketMass();
            request.setCside(store.forces.getCside());
            request.setSin(store.thetaRotation.sin);
            request.setCos(store.thetaRotation.cos);
            Quaternion quaternion = status.getRocketOrientationQuaternion();
            request.setOrientationQuaternion(new Coordinate(quaternion.getW(), quaternion.getX(), quaternion.getY(), quaternion.getZ()));
            request.setCm(store.forces.getCm());
            request.setCyaw(store.forces.getCyaw());
            request.setCroll(store.forces.getCroll());
            request.setLaunchRodCleared(status.isLaunchRodCleared());
            request.setLaunchRodDirection(status.getLaunchRodDirection());
        }
        ...
        if (store.flightConditions.getAOA() != 0 && store.flightConditions.getTheta() != 0) {
            request.setThrustForce(store.thrustForce);
            request.setRocketMass(store.rocketMass);
        }
        ...
        if (store.flightConditions.getAOA() != 0 && store.flightConditions.getTheta() != 0) {
            request.setGravity(store.gravity);
            request.setCoriolisAcceleration(store.coriolisAcceleration);
        }
