...
import net.sf.openrocket.utils.educoder.GlidingCharacter;
import net.sf.openrocket.utils.educoder.GlidingDistance;
...
public class BasicEventSimulationEngine implements SimulationEngine {
    ...
    public FlightData simulate(SimulationConditions simulationConditions) throws SimulationException {
        ...
        //计算结束
        //获取height
        List<FlightEvent> events = flightData.getBranch(0).getEvents();
        //获取最后一次分离时间
        double lastTime = 0.0;
        double lastAltitude = 0.0;
        double lastMach = 0.0;
        double lastAOA = 0.0;
        double lastRefArea = 0.0;
        double c_d = 0.0;
        double c_l = 0.0;
        ...
        List<Double> mach = flightData.getBranch(0).get(FlightDataType.TYPE_MACH_NUMBER);
        ...
        if ((lastMach < 0.05) && (lastAOA > Math.PI / 4)) {
            mul = pow2(lastMach / 0.05);
        }
        //c_l

        //获取所有对称组件
        List<RocketComponent> allChildren = simulationConfig.getRocket().getAllChildren();
        ArrayList<Double> areas = new ArrayList<>();
        for (RocketComponent c : allChildren) {
            if (c instanceof SymmetricComponent) {
                areas.add(((SymmetricComponent) c).getComponentPlanformArea());
            }
        }
        for (Double planFormArea : areas) {
            lastAOA = MathUtil.clamp(lastAOA, 0, Math.PI);
            double sinAOA = 0;
            double sincAOA = 0;
            if (lastAOA < 0.001) {
                sinAOA = lastAOA;
                ...
                sinAOA = Math.sin(lastAOA);
                sincAOA = sinAOA / lastAOA;
            }
            c_l += mul * 1.1 * planFormArea / lastRefArea *
                    sinAOA * sincAOA;
        }
        System.out.println("height: " + lastAltitude);
        ...
        for (int i = 0; i < time.size(); i++) {
            double cl = 0.0;
            double mul2 = 1;
            if ((mach.get(i) < 0.05) && (aoa.get(i) > Math.PI / 4)) {
                mul2 = pow2(mach.get(i) / 0.05);
            }
            ArrayList<Double> componentArea = new ArrayList<>();

            for (RocketComponent c : allChildren) {
                if (c instanceof SymmetricComponent) {
                    componentArea.add(((SymmetricComponent) c).getComponentPlanformArea());
                }
            }
            for (Double planFormArea : componentArea) {
                ...
                if (aoa2 < 0.001) {
                    ...
                    sinAOA = Math.sin(aoa2);
                    sincAOA = sinAOA / aoa2;
                }
                cl += mul2 * 1.1 * planFormArea / refArea.get(i) *
                        sinAOA * sincAOA;
            }
            Cls.add(cl);
        }
